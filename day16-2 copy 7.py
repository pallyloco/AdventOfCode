...
#        print (t,state.key,state.flow)
    total = max_flow_cost*time - node_obj.cost
#    print (time, total, path)
    if total > final_max_flow:
    #    print (time, total, path)
        print (time, node_obj.cost, total, path)
...
# ============================================================================
    ...
    def update_children (self,state):
        ...
        #print (f"******* Parent ******* {state.key}")
        # open the vent (will only work if it has already been turned)
        new_vent_state = VentState.open(you_vent,new_vent_state)
  #      print (f"updated vent_state      {new_vent_state,}")
        new_vent_state = VentState.open(elephant_vent,new_vent_state)
  #      print (f"updated vent state       {new_vent_state}")

        # find all possible children
#        been_there_done_that = list()
        for you_vc in you_vent.children_and_self():
            if you_vc == you_vent and not you_vc.rate: continue
            if you_vc == you_vent and VentState.is_open(you_vent,vent_state): continue

            for elephant_vc in elephant_vent.children_and_self():
                #if not elephant_vc.rate: continue
                if elephant_vc == elephant_vent and VentState.is_open(elephant_vent,vent_state): continue 
#                one,two =  sorted( (you_vc.key,elephant_vc.key) ) 
#                if f"{one}-{two}" in been_there_done_that: continue
#                been_there_done_that.append(f"{one}-{two}")


                # turning a vent
                if you_vc.key == you_vent.key: 
                    new_vent_state = VentState.turn(you_vc,new_vent_state)
                if elephant_vc.key == elephant_vent.key: 
                    new_vent_state = VentState.turn(elephant_vent,new_vent_state)
...
# ============================================================================
    ...
    def eta(self,node):
        ...
        # simple additional cost, is how long does it take to get to the closest vent
        _,cost = self.states.flow_and_costs(this_state.vent_state)
#        cost -=  (you_vent.rate + ele_vent.rate)

        time_left=MAX_MINS - node.time_least_visited        
        wasted_potential = cost*time_left
        

        for time,vent in (c for c in self.states.vents.nearest_vents[you_vent.key] if c[0]<time_left):
            if not VentState.is_open(vent,this_state.vent_state):
                wasted_potential = min(wasted_potential,(time-1)*cost-vent.rate)
        for time,vent in (c for c in self.states.vents.nearest_vents[ele_vent.key] if c[0]<time_left):
            if not VentState.is_open(vent,this_state.vent_state):
                wasted_potential = min(wasted_potential,(time-1)*cost-vent.rate)

        if  not VentState.is_open(you_vent,this_state.vent_state) and not VentState.is_open(ele_vent,this_state.vent_state):
            wasted_potential = cost - you_vent.rate - ele_vent.rate

        elif not VentState.is_open(you_vent,this_state.vent_state):
            wasted_potential = cost - you_vent.rate
        elif not VentState.is_open(ele_vent,this_state.vent_state):
            wasted_potential = cost - ele_vent.rate
        
#        if wasted_potential == MAX_MINS*cost:
#            wasted_potential = cost * time_left

#        if node.time_least_visited == 5:
#            pass
...
# ============================================================================
    ...
    def list_of_nearest_vents (self,vent):
        ...
        for node in shortest_paths :
            if node.obj.rate != 0:
                self.nearest_vents[vent.key].append( (node.time_least_visited, node.obj) )
    
    def fcost(self,vent,vent_state):       
        all = list ( l*v.rate for l,v in self.nearest_vents[vent.key] if VentState.is_closed(v,vent_state) )
        if not VentState.is_open(vent,vent_state):
            all.append(vent.rate)
        return all
...
class VentState:
    @classmethod
    def new (self,vents):
        return "C"*(vents.num_vents())
    @classmethod
    def open(self,vent,vent_str):
        if vent.rate and VentState.is_turning(vent,vent_str):
            return vent_str[:vent.index] + 'O' + vent_str[vent.index+1:]
        return vent_str
    @classmethod
    def turn(self,vent,vent_str):
        if vent.rate and VentState.is_closed(vent,vent_str):
            return vent_str[:vent.index] + 'T' + vent_str[vent.index+1:]
        return vent_str
    @classmethod
    def is_open(self,vent,vent_str):
        return vent_str[vent.index] == 'O'
    @classmethod
    def is_closed(self,vent,vent_str):
        return vent_str[vent.index] == 'C'
    @classmethod
    def is_turning(self,vent,vent_str):
        return vent_str[vent.index] == 'T'
    @classmethod
