        ...
        foreach my $child_obj ( @{ $current->obj->children } ) {
            ...
            my $child_node =
              Node->new( $child_obj, $current->cost + $child_obj->cost,
                         $current );
            next if $self->_already_visited($child_node);
...
sub _update_node {
    ...
    my $current       = shift;
    my $new_node      = shift;
    my $existing_node = $self->_get_to_be_visited_node($new_node);
...
sub _update_cost_array {
    ...
    push @{$costs->{$node->cost + $node->hcost}}, $node;
    $costs->{$node->cost + $node->hcost} = [sort {$a->hcost <=> $b->hcost } @{$costs->{$node->cost + $node->hcost}}];
    return $self;
}
