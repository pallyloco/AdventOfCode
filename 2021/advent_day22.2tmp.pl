...
while ( my $line = <DATA> ) {
    ...
    if ( $last_action eq $action ) {
          merge( $updated_ranges[-1], [ $action, $xrange, $yrange, $zrange ] );
    }
    else {
...
foreach my $range_set (@updated_ranges) {
    my $action = $range_set->[0];
    my $xrange = $range_set->[1];
    my $yrange = $range_set->[2];
    my $zrange = $range_set->[3];

    print "$xrange\t$yrange\t$zrange\n";
    my $set = $action eq "on" ? 1 : 0;
    foreach my $x ( eval ( $xrange ) ) {
        foreach my $y ( eval ( $yrange ) ) {
            foreach my $z ( eval ( $zrange ) ) {
                $reboot{$x}{$y}{$z} = $set;
            }
        }
...
sub count {
    ...
    my $count = 0;
    foreach my $x ( keys %$hash3d ) {

        foreach my $y ( keys %{ $hash3d->{$x} } ) {
            foreach my $z ( keys %{ $hash3d->{$x}{$y} } ) {
                $count += $hash3d->{$x}{$y}{$z};
            }
        }
...
sub merge {
    my $ranges1 = shift;
    my $ranges2 = shift;
    $ranges1->[1] = merge_single($ranges1->[1],$ranges2->[1]);    
    $ranges1->[2] = merge_single($ranges1->[2],$ranges2->[2]);
    $ranges1->[3] = merge_single($ranges1->[3],$ranges2->[3]);
}
...
sub merge_single {
    ...
    foreach my $set (@range_sets) {
        ...
        if (
            !$done && ( $start >= $set_start && $start <= $set_end )
            ||
    ...
    push @new_sets,$range2 unless $done;
