...
foreach my $i ( 1 .. $times ) {
    my @enhanced_image;
    my $symbol = 0;
    $symbol = ($i)%2 if $decode[0];
    print "Processing $i, $symbol\n";
...
sub reset_border {
    ...
    foreach my $row ((0..$size-1),($self->height-$size..$self->height)) {
        foreach my $col (0.. $self->width) {
            $self->map->[$row][$col] = $value;
        }
    }
    foreach my $row ((0..$self->height)) {
        foreach my $col (0.. $size-1,$self->width-$size,$self->width) {
            $self->map->[$row][$col] = $value;
        }
    }
}
