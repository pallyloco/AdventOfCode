...
class AStar:
        ...
        for iteration in it.count():
            ...
            if (self.max_iterations is None or iteration < self.max_iterations) and \
                ...
                for child_obj in current.obj.children():
                    ...
                    cost = current.cumulative_cost + child_obj.edge_cost(current.obj)
                    child_node = Node(child_obj, cost, current)
                    child_node.add_prev(current)
    ...
    # get_path
    # -------------------------------------------------------------------------
    def get_path(self, node, max_nodes=20000) -> list[Node]:
        count = 0
        nodes = [node]
    ...
    # get all best paths
    # -------------------------------------------------------------------------
    def all_best_paths(self, node: Node):
        all_paths = []
        self._all_best_paths( node, all_paths)
        return all_paths

    def _all_best_paths(self, node: Node, all_paths, path=None):
        if path is None:
            path = []
        prev_nodes = node.all_prev
        path.insert(0, node)

        for next_node in prev_nodes:
            new_path = path.copy()
            self._all_best_paths(next_node, all_paths, new_path)
        if len(prev_nodes) == 0:
            all_paths.append(path)
            print (f"Appending {path} to all paths {id(all_paths)} {all_paths}")
    ...
    def _update_node(self, new_node: Node, prev_node: Node):
        ...
        if new_node.id not in self.all_nodes:
            ...
            if updated_cost < new_node.cumulative_cost:
                ...
                new_node.all_prev = [prev_node]
            elif updated_cost == new_node.cumulative_cost:
...
class Node:
    ...
    def add_prev(self, prev: Node):
        self.all_prev.append(prev)
