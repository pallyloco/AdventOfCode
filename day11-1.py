...
class Monkey:
    ...
    def __str__(self):
        str = f"id:{self.id} num_inspected_items: {self.num_inspected_items} items:{self.items} operation:`{self.operation}` test:{self.test} if_true:{self.if_true} if_false:{self.if_false}"
        return str
    def __repr__(self):
        return self.__str__()
