...
# calculate manhatten distances of each scannar pair
my $max_dist = 0;
foreach my $i (0.. scalar(@scanner_locations)-2) {
    foreach my $j ($i+1 .. scalar(@scanner_locations)-1) {
        my $dist = abs($scanner_locations[$i][0] - $scanner_locations[$j][0]);
        $dist += abs($scanner_locations[$i][1] - $scanner_locations[$j][1]);
        $dist += abs($scanner_locations[$i][2] - $scanner_locations[$j][2]);
        $max_dist = $dist if $dist > $max_dist;
    }
}


print "\n\nFINAL ANSWER: $max_dist\n";
...
sub convert {
    ...
    if (@$matches) {
        ...
        # ==============================================================
        # we have 12 matches or more
        # ... for this data, don't need to double check, but this is
        #     a safety precaution
        # ==============================================================
        
        if ( $number > 11 ) {
            push @scanner_locations,[$dx,$dy,$dz];
