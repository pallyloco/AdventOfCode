...
#    print (time_step, total, path)
    ...
    #    print (time_step, total, path)
        ...
        print (time_step, node_obj.cost, total, total_time, path)
...
# ============================================================================
    ...
    def eta(self,node):
        ...
        # for each open valve, how long will it take to get there, and what is the wasted potential
        updated_flow = cost 
        if VentState.is_turning(ele_vent,this_state.vent_state):
            updated_flow -= ele_vent.rate
        if VentState.is_turning(you_vent,this_state.vent_state):
            updated_flow -= you_vent.rate
        ...
        num_valves_open=len(unique_valves_open.values())
        ...
        if len(nearest_valves):
            ...
            new_wasted_potential = min_time*updated_flow
            updated_flow -= vent.rate
            new_wasted_potential += (num_valves_open)*updated_flow
            wasted_potential = new_wasted_potential



        # if the valve has just turned, update the wasted potential accordingly
        if  not VentState.is_open(you_vent,this_state.vent_state) and not VentState.is_open(ele_vent,this_state.vent_state):
            wasted_potential = min(wasted_potential,cost - you_vent.rate - ele_vent.rate)

        elif not VentState.is_open(you_vent,this_state.vent_state):
            wasted_potential = min(wasted_potential,cost - you_vent.rate )
