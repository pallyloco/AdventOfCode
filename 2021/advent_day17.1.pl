...
my $min_x_velocity = int( ( -1 + sqrt( 1 + 8 * $target_limits[0]->col ) ) / 2 +0.9999999999);
...
my $hit = throw( $min_x_velocity, $max_y_velocity, \@target_limits );
if ($hit) {
    print "HIT: \n","max height ", $max_y_velocity * ( $max_y_velocity + 1 ) / 2, "\n";
}
