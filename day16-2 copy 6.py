...
# ============================================================================
    ...
    def eta(self,node):
        ...
        costs_to_next_closed_valves = []

        you_vent = self.states.vents.get(self.you_vent_key)
        ele_vent = self.states.vents.get(self.elephant_vent_key)

        fcosts = self.states.vents.fcost(you_vent,self.vent_state)+self.states.vents.fcost(ele_vent,self.vent_state)
        
        minimal_cost = 0
        costs_to_next_closed_valves = sorted(fcosts)
        try:
            minimal_cost = minimal_cost+costs_to_next_closed_valves[0]
        except Exception:
            return 0
        try:
            minimal_cost += minimal_cost+costs_to_next_closed_valves[1]
            return int(minimal_cost/2)
        except Exception:
            return minimal_cost/2
...
# ============================================================================
    ...
    def fcost(self,vent,vent_state):       
        all = list ( (l+1)*v.rate for l,v in self.nearest_vents[vent.key] if v.is_closed(vent_state) and v.rate !=0)
        if vent.is_closed and vent.rate !=0:
...
# ============================================================================
    ...
    def open_vent (self,vent_state):
        return vent_state + (1<<self.index)
