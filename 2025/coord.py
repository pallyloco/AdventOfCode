...
class DataInCell:
    ...
    def rotate_90_counter_clockwise(self):
        """changed the direction by rotating 90 degrees counterclockwise"""
    ...
    def direction_to(self, other) -> Literal[
    ...
    def between(self, cmin: DataInCell, cmax: DataInCell) -> bool:
        """are you in between two coordinates"""
        return cmin.row <= self.row <= cmax.row and cmin.col <= self.col <= cmax.col

    def scale(self, scale: int) -> DataInCell:
        """change scale"""
        return DataInCell(int(self.row * scale), int(self.col * scale))

    def __sub__(self, other) -> DataInCell:
        return DataInCell(self.row - other.row, self.col - other.col, self.value)

    def __eq__(self, other) -> bool:
        return self.row == other.row and self.col == other.col

    def __add__(self, other) -> DataInCell:
        return DataInCell(self.row + other.row, self.col + other.col, self.value)
