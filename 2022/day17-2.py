...
num_rocks_falling = 1000000000000
chamber_width = 7
repeated = {}
...
def main():
    ...
    for r in range(num_rocks_falling):
        ...
        if not r%num_rocks:
            ...
            print (r+repeated_rock, "   ", end="")
            for bit in range(chamber_width-1,-1,-1):
                if 1<<bit & chamber[current_height]:
                    print ("#",end="")
                else:
                    print (".",end="")
            print(f"   {current_height+offset_height}  wind:{wind_num%num_wind_gusts}")

            if not repeated_rock: 
                key = f"{chamber[current_height]}-{wind_num%num_wind_gusts}"
                if key not in repeated:
                    repeated[key] = list()
                repeated[key].append((current_height, r))

                if len (repeated[key]) >= 2:
                    l_height,l_num_rock = repeated[key][-1]
                    p_height,p_num_rock = repeated[key][-2]

                    d_height = l_height - p_height
                    d_num_rocks = l_num_rock - p_num_rock
                    num_repeats = (num_rocks_falling-l_num_rock) // d_num_rocks
                    offset_height = num_repeats*d_height
                    repeated_rock = num_repeats*d_num_rocks

        if r+repeated_rock >= num_rocks_falling:
            break
    ...
    print(r+repeated_rock,current_height+offset_height)        
