...
# ============================================================================
    ...
    def update_children (self,state):
        ...
        new_vent_state = VentState.open_any_turned_valves(new_vent_state)
        ...
        for you_vc in you_kids:
            for elephant_vc in ele_kids:
                ...
                if you_vc.key == you_vent.key and elephant_vc.key == ele_vent.key: 
                    ...
                    flow, cost = self.flow_and_costs(tmp_vent_state)
                    state.add_child( State(tmp_vent_state, you_vc.key, elephant_vc.key,flow,cost,self))
                if elephant_vc.key == ele_vent.key: 
                    tmp_vent_state = VentState.turn(elephant_vc,new_vent_state)
                    flow, cost = self.flow_and_costs(tmp_vent_state)
                    state.add_child( State(tmp_vent_state, you_vc.key, elephant_vc.key,flow,cost,self))
...
#        cost -=  (you_vent.rate + ele_vent.rate)
        ...
        for time,vent in (c for c in self.states.vents.closest_vents[you_vent.key] if c[0]<time_left):
            if not VentState.is_open(vent,this_state.vent_state):
                wasted_potential = min(wasted_potential,(time-1)*cost-vent.rate)
        for time,vent in (c for c in self.states.vents.closest_vents[ele_vent.key] if c[0]<time_left):
            if not VentState.is_open(vent,this_state.vent_state):
                wasted_potential = min(wasted_potential,(time-1)*cost-vent.rate)
...
# ============================================================================
    ...
    def _list_of_nearest_vents (self,vent):
        ...
        for node in shortest_paths :
            if node.obj.rate != 0:               
                self.nearest_vents_lookup[vent.key][node.id] = (node.time_least_visited, [n.id for n in dijkstra.get_path(node)] ) 
